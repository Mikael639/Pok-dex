...
function App() {
  ...
  }, [activeTab, gameState.target, memoryState.cards.length, pokemons.length, quizState.typeA, ensureActiveExperienceReady]);
  ...
  const suggestionPokemons = useMemo(
    () =>
      pokemons
        .filter((pokemon) => pokemon.nom.toLowerCase().includes(searchQuery.toLowerCase()))
        .slice(0, 5),
      ...
      <aside aria-label="Navigation principale" aria-hidden={!isSidebarVisible} {...(!isSidebarVisible ? { inert: true } : {})} className={`fixed inset-y-0 left-0 z-[70] transition-all duration-700 transform border-r-[6px] flex flex-col ${isSidebarOpen ? 'w-80' : 'w-24'} ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${isDarkMode ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-100 shadow-2xl text-slate-900'}`}>
        ...
        <nav className="p-4 space-y-1 overflow-y-auto custom-scrollbar">
            {[
              ...
              <button type="button" key={item.id} onClick={() => { setActiveTab(item.id); setIsMobileMenuOpen(false); }} aria-current={activeTab === item.id ? 'page' : undefined} aria-label={item.count !== undefined ? `${item.label} ${item.count}/6` : item.label} className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-[1.5rem] font-black transition-all duration-300 relative group ${activeTab === item.id ? 'bg-rose-500 text-white shadow-2xl shadow-rose-500/40 scale-[1.02]' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}>
          ...
          {activeTab === 'accueil' && (
            <Motion.div key="h" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} className="space-y-12 pb-20">
                  ...
                  <div className="xl:col-span-2 relative rounded-[2.5rem] lg:rounded-[4rem] overflow-hidden group shadow-2xl h-[300px] lg:h-[450px] bg-slate-900 border-4 border-white dark:border-slate-800">
                    ...
                    <div className="relative h-full flex flex-col justify-end p-6 lg:p-12 space-y-4 lg:space-y-6">
                       ...
                       <div className="flex gap-3 lg:gap-4">
                          <button type="button" onClick={() => setActiveTab('collection')} className="bg-white text-slate-950 px-5 lg:px-8 py-3 lg:py-4 rounded-xl lg:rounded-2xl font-black text-[10px] lg:text-sm shadow-2xl hover:bg-rose-500 hover:text-white transition-all uppercase tracking-widest">Voir Index</button>
                          <button type="button" onClick={() => setActiveTab('combat')} className="bg-slate-800/40 backdrop-blur-xl border border-white/20 text-white px-5 lg:px-8 py-3 lg:py-4 rounded-xl lg:rounded-2xl font-black text-[10px] lg:text-sm shadow-2xl hover:bg-white hover:text-slate-900 transition-all uppercase tracking-widest">Combat</button>
                       </div>
                    </div>
                  </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <button type="button" aria-label="Afficher le Pokemon du jour" onClick={() => (dailyFeaturedPokemon ? setSelectedPokemon(dailyFeaturedPokemon) : getRandomPokemon())} className="holographic glass-card rounded-[3.5rem] p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:scale-[1.02] transition-all group border-2 border-transparent hover:border-amber-400/30">
                     ...
                     {dailyFeaturedPokemon && <div className="relative"><div className="absolute inset-0 bg-amber-400/20 blur-3xl rounded-full scale-150 animate-pulse" /><img src={dailyFeaturedPokemon.image} alt={dailyFeaturedPokemon.nom} className="w-32 h-32 object-contain drop-shadow-2xl relative z-10 group-hover:rotate-12 transition-transform duration-500" /></div>}
                  ...
                  <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="bg-white dark:bg-slate-900 border-4 border-transparent hover:border-indigo-500/20 p-8 rounded-[3.5rem] shadow-xl transition-all relative overflow-hidden">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                           {team.length > 0 ? team.map(p => <button type="button" key={p.id} aria-label={`Voir ${p.nom}`} className="relative bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl text-center border-2 border-transparent hover:border-indigo-500 transition-all cursor-pointer" onClick={() => setSelectedPokemon(p)}><img src={p.image} alt={p.nom} className="w-12 h-12 mx-auto object-contain hover:scale-110 transition-transform" /></button>) : <div role="status" className="col-span-3 py-6 text-center opacity-30 italic font-black text-xs uppercase tracking-widest">Équipe vide...</div>}
                        </div>
                     </div>
